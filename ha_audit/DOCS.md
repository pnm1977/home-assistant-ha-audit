...
HA Audit generates detailed machine-readable reports in its private app storage.
...
`ha_audit_latest.txt` is overwritten every run.

The JSON files contain the detailed data used to build the summary.

They are primarily diagnostic/internal reports at this stage of development.

The normal user workflow should start with:

**Settings → Apps → HA Audit → Log**

Future versions may make the detailed reports easier to browse directly.
